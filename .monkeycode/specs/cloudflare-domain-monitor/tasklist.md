...
| 编号 | 任务 | 状态 | 完成度 | 子任务文档 |
|------|------|------|--------|-----------|
| 28 | 公开域名详情页 | 🔴 待开始 | 0% | `subtask-28-domain-detail.md` |
...
### 遗留功能（未实现）
1. **域名详情页**（P2）：公开页面点击域名应跳转到详情页展示完整检测结果与历史趋势。`PublicDashboard.js` 第 371 行有 `// TODO: 跳转到域名详情页（任务 16）`
2. **安全配置 UI**（P3）：后端 `GET /api/admin/config/security` 已实现，前端无对应管理界面

### 技术债（task-18-todo.md）
| 编号 | 描述 | 工时 |
|------|------|------|
| TECH-18-01 | AdminConfig render 方法过长（394 行→拆分为 Section 组件） | 1h |
| TECH-18-04 | URL 验证 polyfill（旧浏览器兼容） | 0.5h |
| TECH-18-05 | DoH 测试超时控制（前端 5s 超时提示） | 1h |
| TECH-18-06 | 配置变更检测（未修改时禁用保存按钮） | 2h |

### 安全改进（建议）
3. **清理遗留公开路由**（P2）：`/api/domains`, `/api/detect/*`, `/api/result/*` 无认证直接暴露，前端已不使用，建议添加认证或标记 deprecated
