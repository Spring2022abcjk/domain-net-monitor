...
export async function runAdminDashboardTests() {
  await runSuite('AdminDashboard File Exists', () => {
    assertEqual(
      existsSync(join(ROOT, 'src/pages/admin/AdminDashboard.js')),
      true,
  ...
  await runSuite('AdminDashboard Class Structure', () => {
    ...
    assertEqual(content.includes('export class AdminDashboard'), true,
      'Exports AdminDashboard class')
  ...
  await runSuite('AdminDashboard Event Handler Refactoring', () => {
    ...
    assertEqual(content.includes('id="dashboard-refresh-btn"'), true,
      'Uses id="dashboard-refresh-btn" instead of onclick')
    assertEqual(content.includes('this.__refreshHandler'), true,
      'Stores handler reference in constructor')
    assertEqual(content.includes("addEventListener('click', this.__refreshHandler)"), true,
      'Uses addEventListener with handler reference')
    assertEqual(content.includes("removeEventListener('click', this.__refreshHandler)"), true,
  ...
  await runSuite('AdminDashboard No window.__ Patterns', () => {
    ...
    assertEqual(content.includes('window.__dashboardRefreshHandler'), false,
  ...
  await runSuite('AdminDashboard Refresh Handler Re-renders', () => {
    ...
    assertEqual(content.includes('admin-content'), true,
      'Refresh handler targets admin-content container')
    assertEqual(content.includes('this.render()'), true,
      'Refresh handler calls this.render() to re-render')
    assertEqual(content.includes('this.bindEvents()'), true,
      'Refresh handler calls this.bindEvents() after re-render')
    assertEqual(content.includes('this.loadData()'), true,
      'Refresh handler calls this.loadData()')
  })
}
