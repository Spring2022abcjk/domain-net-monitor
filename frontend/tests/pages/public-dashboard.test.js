...
export async function runPublicDashboardTests() {
  ...
  await runSuite('Pages - PublicDashboard Exists', async () => {
    assertEqual(existsSync(join(frontendRoot, 'src/pages/PublicDashboard.js')), true, 'PublicDashboard.js exists')
  })
  ...
  await runSuite('Pages - PublicDashboard Component', async () => {
    ...
    assertEqual(
      page.includes("window.location.hash = '#/domain/'"),
      true,
      'handleViewDetail navigates to domain detail page via hash',
    )
    assertEqual(page.includes('encodeURIComponent(domain)'), true, 'Encodes domain name in hash URL')
