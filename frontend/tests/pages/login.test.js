...
export async function runLoginTests() {
  ...
  await runSuite('Task 15 - Login Files Exist', async () => {
    assertEqual(existsSync(join(frontendRoot, 'src/pages/Login.js')), true, 'Login.js exists')
  })
  ...
  await runSuite('Task 15 - Uses New Component Library', async () => {
    ...
    assertEqual(
      loginCode.includes("import { Button } from '../components/Button.js'"),
      true,
      'Imports Button component',
    )
    assertEqual(loginCode.includes("import { Card } from '../components/Card.js'"), true, 'Imports Card component')
    assertEqual(
      loginCode.includes("import { show } from '../components/Notification.js'"),
      true,
  ...
  await runSuite('Task 15 - Router Guard', async () => {
    ...
    // 检查认证检查
    assertEqual(routerCode.includes('requiresAuth'), true, 'Checks requiresAuth for protected routes')
  })
