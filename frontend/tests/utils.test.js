...
async function runUtilsTests() {
  ...
  await runSuite('Utils - formatDate', async () => {
    ...
    assertEqual(formatDate(date, 'YYYY-MM-DD'), '2026-06-01', 'Formats date to YYYY-MM-DD')
    assertEqual(formatDate(date, 'YYYY/MM/DD HH:mm:ss'), '2026/06/01 12:30:45', 'Formats date with custom format')
    assertEqual(formatDate(date), '2026-06-01 12:30:45', 'Uses default format')
  })
  ...
  await runSuite('Utils - formatRelativeTime', async () => {
    ...
    assertEqual(formatRelativeTime(now).includes('刚刚'), true, 'Returns 刚刚 for current time')
    assertEqual(formatRelativeTime(fiveMinutesAgo).includes('分钟前'), true, 'Returns 分钟前 for recent time')
    assertEqual(formatRelativeTime(twoHoursAgo).includes('小时前'), true, 'Returns 小时前 for hours ago')
    assertEqual(formatRelativeTime(threeDaysAgo).includes('天前'), true, 'Returns 天前 for days ago')
  })
