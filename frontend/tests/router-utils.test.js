...
async function runRouterUtilsTests() {
  ...
  await runSuite('Router Utils - matchRoute', async () => {
    ...
    assertEqual(matchRoute('/admin', '/admin/dashboard'), null, 'Returns null for mismatched paths')
  ...
  if (typeof window !== 'undefined') {
    await runSuite('Router Utils - navigate', async () => {
      ...
      assertEqual(window.location.hash.includes('q=test'), true, 'Adds query params')
      assertEqual(window.location.hash.includes('page=2'), true, 'Adds multiple query params')

      navigate('/domain/:name/detail', { name: 'example.com' }, { tab: 'settings' })
      assertEqual(window.location.hash, '#/domain/example.com/detail?tab=settings', 'Combines params and query')

      navigate('/encoded/:name', { name: 'a/b' })
      assertEqual(window.location.hash.includes('a%2Fb'), true, 'Encodes special characters in params')
