...
async function testCorsWhitelistScenarios() {
  await runSuite('CORS whitelist scenarios', async () => {
    ...
    assertEqual(headers1['Access-Control-Allow-Origin'], 'https://your-single.your-domain.pages.dev', 'Prod single domain - match');
    assertEqual(headers1['Vary'], 'Origin', 'Prod single domain - Vary header');
    
    // 场景 2: 生产环境 - 多域名白名单
    const env2 = createMockEnv({ 
      ALLOWED_ORIGINS: 'https://your-single.your-domain.pages.dev,https://dev.your-domain.pages.dev,https://staging.your-domain.pages.dev' 
    });
    ...
    assertEqual(headers2a['Access-Control-Allow-Origin'], 'https://dev.your-domain.pages.dev', 'Multi domain - match dev');
    ...
    assertEqual(headers2b['Access-Control-Allow-Origin'], 'https://staging.your-domain.pages.dev', 'Multi domain - match staging');
...
async function testOptionsPreflight() {
  await runSuite('OPTIONS preflight responses', async () => {
    ...
    assertEqual(response1.headers.get('Access-Control-Allow-Methods'), 'GET, POST, PUT, DELETE, OPTIONS', 'OPTIONS wildcard - methods');
    assertEqual(response1.headers.get('Access-Control-Allow-Headers'), 'Content-Type, X-API-Token', 'OPTIONS wildcard - headers');
    ...
    assertEqual(response2.headers.get('Access-Control-Allow-Origin'), 'https://your-single.your-domain.pages.dev', 'OPTIONS whitelist - origin');
