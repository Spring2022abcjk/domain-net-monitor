...
export async function runFeatureTests() {
  ...
  await runSuite('detectScheduled - Detect default domains', async () => {
    ...
    await env.DOMAIN_MONITOR_KV.put('domain_list', JSON.stringify(['cloudflare.com', 'google.com']))
  ...
  await runSuite('detectScheduled - Domain detection results format', async () => {
    ...
    // 设置默认域名
    await env.DOMAIN_MONITOR_KV.put('default_domains', JSON.stringify(['cloudflare.com']))
    await env.DOMAIN_MONITOR_KV.put('domain_list', JSON.stringify(['cloudflare.com']))
  ...
  await runSuite('detectScheduled - Domain detection fails gracefully', async () => {
    ...
    // 设置一个无效的默认域名
    await env.DOMAIN_MONITOR_KV.put('default_domains', JSON.stringify(['invalid-domain-that-does-not-exist-12345.com']))
    await env.DOMAIN_MONITOR_KV.put('domain_list', JSON.stringify(['invalid-domain-that-does-not-exist-12345.com']))
